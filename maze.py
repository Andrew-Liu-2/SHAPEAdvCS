...
class MazeVisualizer(object):
    def __init__(self): 
      ...
      self.root = root
      self.canvas = Canvas(root,height = self.cell_size * self.height, width = 
self.cell_size * self.width, borderwidth=0, highlightthickness=0)
      self.canvas.pack()
    def draw_grid(self):
      for i in range(self.height):
        for j in range(self.width):
          self.canvas.create_rectangle(j*self.cell_size, i*self.cell_size, 
(j+1)*self.cell_size, (i+1)*self.cell_size, outline="gray", fill="white") 
      
    def draw_maze(self, maze):
      self.canvas.create_rectangle(0,0,self.width * self.cell_size-1, self.height *
self.cell_size-1, outline="black") # outline
      for i in range(self.height): 
        for j in range(self.width): 
          cell = maze[(i,j)]
          if cell[0]: self.canvas.create_line(j*self.cell_size, i*self.cell_size, 
j*self.cell_size, (i+1)*self.cell_size, width=2)    # left
          if cell[1]: self.canvas.create_line(j*self.cell_size, i*self.cell_size, 
(j+1)*self.cell_size, i*self.cell_size, width=2)    # up 
          if cell[2]: self.canvas.create_line((j+1)*self.cell_size, 
i*self.cell_size, (j+1)*self.cell_size, (i+1)*self.cell_size, width=2)    # right  
          if cell[3]: self.canvas.create_line(j*self.cell_size, 
(i+1)*self.cell_size, (j+1)*self.cell_size, (i+1)*self.cell_size, width=2)    # down
...
def paint_cell(self,y,x, color): 
      self.canvas.create_oval(self.cell_size * x+5, self.cell_size * y+5, 
self.cell_size* (x+1)-5, self.cell_size * (y+1)-5, fill=color, outline=color)
